...
// ============================================================
  ...
  async function exportSingleChat() {
    ...
    try {
      ...
      const safeName = String(chat.name || '未命名聊天').replace(/[\\/:*?"<>|]/g, '_');
      const dateStr = new Date().toISOString().split('T')[0];

      const getExportMessageText = (msg) => String(
        msg && msg.content !== undefined && msg.content !== null ? msg.content :
        msg && msg.text !== undefined && msg.text !== null ? msg.text :
        msg && msg.message !== undefined && msg.message !== null ? msg.message :
        ''
      );
...
`;
        if (exportableHistory.length > 0) {
          ...
          for (const msg of exportableHistory) {
            ...
            const rawText = getExportMessageText(msg);
            const safeRawText = escapeAttr(rawText);
            const safeContentHtml = escapeHtml(rawText);
            const safeMeta = escapeAttr(`${role} · ${time}`);
            const safeMetaHtml = escapeHtml(`${role} · ${time}`);

            htmlContent += `
        <div class="${msgClass}">
          <div class="avatar-container"><div class="avatar" style="background:${avatarBg};">${initial}</div></div>
          <div class="msg-body">
            <div class="meta">${safeMetaHtml}</div>
