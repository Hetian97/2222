...
# 提取原则：
- 只提取稳定、具体、未来仍可能有用的信息。
- 不要记录寒暄、重复内容、无意义测试或短暂无影响的日常。
- 同一场景或同一话题尽量合并成一条完整记忆，不要把连续事件拆得过碎。
- 每条记忆应围绕一个核心主题，长度尽量控制在 50-120 字；重要核心设定或复杂关系节点可以稍长。若同一场景中存在两个以上互相独立、以后可能分别被检索的主题，应拆成 2 条。
- 不要为了变短而拆碎同一主题下的连续动作；应按“主题”拆分，而不是按动作、句子或镜头拆分。
- 保留关键时间、地点、人物、承诺、关系变化、重要事件和稳定偏好。
- 不要把一次性行为改写成“习惯”“总是”“长期如此”“以后都会”等长期模式，除非原文明确表达了持续性。
- 如果原文中的“她/他/对方”指代不清，且事件本身不重要，不要强行提取。
- 如果同一事实、同一设定、同一地点或同一事件已经存在于长期记忆中，只有出现新的信息、关系变化、承诺升级、情绪变化或重要进展时才新增记忆。不要仅因为再次提到同一件事就重复记录。
- E 不是“低价值分类”。如果一条记忆主要记录一次具体发生过的共同场景，即使其中包含亲密、照顾、调侃、普通承诺或短期安排，只要这些内容服务于同一场景，通常归为 E，而不是拆散到 R/P/A/M。

# 10大精细分类说明：
请优先判断这条记忆的核心是什么。若核心是一次具体共同经历、一次场景节点、一次有回忆价值的日常互动，优先归为 E；只有当核心明显是稳定设定、长期规则、重要物品、关系转折、强烈心理状态、核心灵魂或尚未完成的计划时，才归入 U/A/R/I/L/P/T/M/C。不要把 E 当作兜底分类，也不要把所有日常都归为 E。
