...
/* ===== SERVER_SIDE_VECTOR_MEMORY_PATCH_V1 =====
   When external memory-server is enabled, use backend /memory/search
   instead of browser-side embeddings.
*/
(function () {
  function getExternalMemoryEndpoint() {
    try {
      const enabled = localStorage.getItem('vm_external_memory_enabled') === 'true';
      if (!enabled) return '';
      return (localStorage.getItem('vm_external_memory_endpoint') || '').trim().replace(/\/+$/, '');
    } catch (e) {
      return '';
    }
  }

  async function fetchServerSideMemoryContext(chat, queryText) {
    const endpoint = getExternalMemoryEndpoint();
    if (!endpoint) return null;

    const response = await fetch(endpoint + '/memory/search', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        query: queryText || '',
        chatId: chat && (chat.id || chat.chatId || ''),
        limit: 12,
        candidateLimit: 1000,
        chromaNResults: 120
      })
    ...
    if (!response.ok) {
      throw new Error('memory-server /memory/search HTTP ' + response.status + ': ' + await response.text());
    }

    const data = await response.json();
    const memories = Array.isArray(data.memories) ? data.memories : [];

    console.log('[vector-memory patched] memory-server search ok:', {
    ...
    return '# 长期记忆（来自 memory-server / Chroma）\n' + memories.map((m) => {
      const extras = [];
      if (m.category) extras.push('分类：' + m.category);
      if (m.keywords) extras.push('关键词：' + m.keywords);
      if (m.importance) extras.push('重要性：' + m.importance);
      if (m.emotionalWeight) extras.push('情绪权重：' + m.emotionalWeight);
      const meta = extras.length ? '（' + extras.join('；') + '）' : '';
      return '- ' + (m.content || m.summary || JSON.stringify(m)) + meta;
    }).join('\n');
  }

  function installPatch() {
    if (!window.vectorMemoryManager || window.vectorMemoryManager.__serverSideMemoryPatched) {
      return false;
    }

    const original = window.vectorMemoryManager.serializeForPrompt;
    if (typeof original !== 'function') return false;

    window.vectorMemoryManager.serializeForPrompt = async function (chat, queryText) {
      const endpoint = getExternalMemoryEndpoint();

      if (endpoint) {
        try {
          return await fetchServerSideMemoryContext(chat, queryText);
        } catch (error) {
          console.warn('[vector-memory patched] memory-server search failed, fallback to original vector memory:', error);
        }
      }

      return await original.call(this, chat, queryText);
    };

    window.vectorMemoryManager.__serverSideMemoryPatched = true;
    console.log('[vector-memory patched] installed: external memory-server will use /memory/search');
    return true;
  }
